...
There are more states above 0 than below 0, which indicates that states are more likely to go down in the rankings (have less cases relative to others) after they quarantine, which makes sense. However, it's not entirely conclusive, which can likely be drawn up to the fact A) that so many states quarantined at about the same time and B) there are a lot of factors outside of just the quarantine date, like simply the rate and stage of growth of surrounding states (the later a Northeastern state quarantined, the lower it is on the graph) as well as unpredictability of who gets tested.

If we instead  wanted to look at the rate of growth of each state before and after quarantining, one way would be to isolate the dates around each quarantine date. If we look at the preceeding 2 weeks and the succeedings two weeks of a select few states, the data appears as so:
